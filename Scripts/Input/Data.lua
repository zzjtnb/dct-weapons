...
local function loadDeviceProfileFromFile(filename, deviceGenericName, folder)
	...
	if f then
		...
		local env = Input.getEnvTable().Actions
		env.folder			 	=	folder
...
local function getKeyDiffs_(profile, deviceName)
	print('~~~getKeyDiffs_', getProfileName_(profile), deviceName)
	local diffs = {}
	local commands = getProfileKeyCommands_(profile)
	...
	for commandHash, command in pairs(commands) do	
		...
		print('~~~~defaultCommand', command.name)
		local combos = command.combos[deviceName]
		
		if combos and combos[1] then
			print('~~~combos count:', #combos)
			print('~~~combo key', combos[1].key)
		else
			print('~~~combo key', '[NO]')
		end
		...
		if defaultCommand then
			...
			print('~~~~commandDiff', command.name, commandDiff)
...
local function saveChanges()
	...
	for i, profile in ipairs(profiles_) do
		if getProfileLoaded_(profile) and getProfileModified_(profile) then
			...
			print('~~~saveChanges', profileName, profileUserConfigPath)
