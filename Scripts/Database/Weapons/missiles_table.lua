...
function form_missile(name, user_name, model, class, level3, scheme, data, add_data,wstype_name)
	...
	-- Это нужно, потому что все параметры, кроме фантомов могли быть заданы
	-- прямо в схеме
...
declare_missile("ALARM", _("ALARM"), "t-alarm", "wAmmunitionSelfHoming", wsType_AS_Missile, "anti_radiation_missile", 
{
	...
	radio_seeker = {
		FOV					= math.rad(6),	-- !	угол обзора
		op_time				= 300,			-- !	время работы датчика
		keep_aim_time		= 5,			-- !	время до начала поиска новой цели после потери сигнала
		pos_memory_time		= 150,			-- !	время до сброса памяти о точке прицеливания
		sens_near_dist		= 150.0,		--		минимальная видимая дистанция	 
		sens_far_dist		= 70000.0,		--		максимальная видимая дистанция
		err_correct_time	= 1.2,			-- !!	период обновления инфо о цели (скорректировать ошибку)
		err_val				= 0.0026,		-- !!	максимальная угловая ошибка
		calc_aim_dist		= 500000,		-- 		максимальное расстояние для потсроения точки прицеливания (служ)
		blind_rad_val		= 0.1,			--		максимальная входная мощность сигнала до засвета (служ) (сейчас ф-ция расстояния)
		blind_ctrl_dist		= 2100,			--		построение доп точки прицеливания на малом расстоянии (служ)
		aim_y_offset		= 3.5,			-- !!	сдвиг прицела по высоте относительно aimPoint выбранной цели
	},
