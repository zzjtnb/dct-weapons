...
GT.WS[ws].angles = {
					{math.rad(180), math.rad(-180), math.rad(-5), 1.2217},
					};
...
GT.WS[ws].base = base;
GT.WS[ws].angles = {
					{math.rad(15), math.rad(-15), math.rad(-5), 1.2217},
					};
GT.WS[ws].omegaY = 1;
GT.WS[ws].omegaZ = 1;
GT.WS[ws].LN = {};
...
GT.WS[ws].angles = {
					{math.rad(2), math.rad(-2), math.rad(-5), 1.2217},
					};
GT.WS[ws].omegaY = 1;
GT.WS[ws].omegaZ = 1;
...
GT.WS[ws].mount_before_move = true;
__LN = add_launcher(GT.WS[ws], GT_t.LN_t._9A310M1);
...
-- ќписываютс€ после ракетного лаунчера дл€ того,
-- чтобы его WS сделать базовой дл€ правильной реализации поисковых возможностей радара пусковой установки
-- (ищем только в узком секторе относительно направлени€ головы).
-- ќсновной луч радара подсвета цели Ќ≈Ћ№«я использовать как базу из-за особенностей реализации
...
GT.WS[ws].base = base;
GT.WS[ws].angles = {
					{math.rad(60), math.rad(-60), math.rad(-5), 1.2217},
					};
GT.WS[ws].omegaY = 1;
GT.WS[ws].omegaZ = 1;
...
GT.Name = "SA-11 Buk LN 9A310M1";
GT.DisplayName = _("SAM SA-11 Buk LN 9A310M1");
GT.Rate = 70;
